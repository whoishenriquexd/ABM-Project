globals [
  ...
  condom-effectiveness     ;; The chance out of 100 that a condom is effective
  ;;treating-percentage      ;; The chance out of 100 that a person starts treatment
  prep-infection-chance
  days-on-prep
...
to setup-globals
  ...
  set condom-effectiveness 85;; 85% chance of a condom being effective
  ;;set treating-percentage 77 ;; 77% are on antiretroviral treatment in Brazil
  set prep-infection-chance 14 ;; percentage of chances divided by 7 days
  set days-on-prep choose-days-on-prep
  ...
  set slider-check-5 average-prep-use
end
...
to assign-prep-use  ;; turtle procedure
  set prep-use random-near average-prep-use
end
...
;;  ask turtles [ kill-susceptibles ]
  ...
  ask turtles [ infect ]
  ;;ask turtles [ infect-prep ]
  ;;ask turtles [ infect-condom ]
  ask turtles [ test ]
...
to check-sliders
  ...
  if (slider-check-4 != average-test-frequency )
    [ ask turtles [ assign-test-frequency ]
      set slider-check-4 average-test-frequency ]
  if (slider-check-5 != average-prep-use )
    [ ask turtles [ assign-prep-use ]
      set slider-check-5 average-prep-use ]
end
...
to infect  ;; turtle procedure
  ifelse random-float 10 < prep-use [
    infect-prep
  ][
    infect-condom
  ]
...
to infect-prep  ;; turtle procedure
    if coupled? and infected? and not known?
     [ if random-float 100 > (prep-infection-chance * days-on-prep)
        [ ask partner [ set infected? true ] ] ]
end

to infect-condom  ;; turtle procedure
    if coupled? and infected? and not known?
    [ if random-float 10 > condom-use or
         random-float 10 > ([condom-use] of partner)
      [ if random-float 100 > condom-effectiveness
...
;;to-report %treating
;;  ifelse any? turtles
;;    [ report (count turtles with [treating?] / count turtles) * 100 ]
;;    [ report 0 ]
;;end
...
<experiments>
  <experiment name="experiment1" repetitions="1" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="choose-days-on-prep">
      <value value="7"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="average-prep-use">
      <value value="8"/>
    </enumeratedValueSet>
